...
class rc_sweet_custom_menu {
	...
	*/
	function rc_scm_add_custom_nav_fields( $menu_item ) {
		...
		$menu_item->mega_menu_column_name = get_post_meta( $menu_item->ID, '_menu_item_mega_menu_column_name', true);
	...
	*/
	function rc_scm_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
        ...
        $enable_mega_menu = isset($_REQUEST['menu-item-enable-mega-menu'], $_REQUEST['menu-item-enable-mega-menu'][$menu_item_db_id]);
		$column_name = isset($_REQUEST['menu-item-mega-menu-column-name']) ? $_REQUEST['menu-item-mega-menu-column-name'][$menu_item_db_id] : '';

        update_post_meta( $menu_item_db_id, '_menu_item_enable_mega_menu', $enable_mega_menu );
        update_post_meta( $menu_item_db_id, '_menu_item_mega_menu_column_name', $column_name );

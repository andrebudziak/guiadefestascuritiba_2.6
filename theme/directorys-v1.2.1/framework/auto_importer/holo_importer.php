...
require dirname( __FILE__ ) . '/wordpress_importer/parsers.php';
 ...
 * @subpackage Importer
 */
if ( ! class_exists( 'Holo_Auto_Importer' ) ) {
    class Holo_Auto_Importer extends WP_Importer
    {
        ...
        function Holo_Auto_Importer( $args ) {
        ...
        function process_categories() {
            ...
            $_SESSION['holo'] = 'Categories has been imported';
        }
...
function auto_import( $args ) {
    ...
    $autoimport = new Holo_Auto_Importer( $args );
...
function import_content() {
    ...
    $original_file = HOLO_TEMPLATEDIR . '/framework/auto_importer/import.xml';
    $dest_file = HOLO_TEMPLATEDIR . '/framework/auto_importer/import-copy.xml';
    ...
    if ( copy($original_file, $dest_file) ) {
        ...
        auto_import( $args );
        ht_save_demo_markers();

    } else {

        die('The import.xml could not be copied');

    }
}

add_action( 'wp_ajax_check_importer_state', 'check_importer_state');
function check_importer_state() {

//    if (empty($_SESSION['holo'])) {
//        $_SESSION['holo'] = 'is empty';
//    }
...
function ht_save_demo_markers() {
    ...
    $site_loc_terms = get_terms('sites_category');
    ...
    foreach( $site_loc_terms as $term ) {
        ...
        $icons_urls = array();
        $site_icon = get_option( "taxonomy_$term->term_id" );

        $category_marker = $site_icon['pin'];
        $category_badge = $site_icon['badge'];

        $site_pin = ht_get_external_image('http://holobest.com/directorys/wp-content/uploads/2014/09/pin-' . $demo_categories[$term->slug] . '.png', $destination . '/pin-' . $term->term_id . '.png' );
        $site_badge = ht_get_external_image('http://holobest.com/directorys/wp-content/uploads/2014/09/badge-' . $demo_categories[$term->slug] . '.png', $destination . '/badge-' . $term->term_id . '.png' );

        if ($site_pin && $site_badge) {

            $pin_dest = $upload_dir['url'] . '/pin-' . $term->term_id . '.png';
            $badge_dest = $upload_dir['url'] . '/badge-' . $term->term_id . '.png';

            $icons_urls['pin'] = $pin_dest;
            $icons_urls['badge'] = $badge_dest;

            if (!get_option('taxonomy_' . $term->term_id)) {

                add_option('taxonomy_' . $term->term_id, $icons_urls );
...
function ht_get_external_image($img_url, $dest) {
//The cURL stuff...
    $ch = curl_init();

...
public partial class Convite : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //if (Session["menu"] == "0")
            //{
                GerMenu(grdMenuDecoracao, "7");
                GerMenu(grdMenuLocais, "8");

                GerMenuSup(dlLocais, "7");
                GerMenuSup(dlDecoracao, "8");
                Session["menu"] = "1";
    ...
    protected void lnk_lnkCategoria_Click(object sender, EventArgs e)
    {
        ...
        Session["tipo_convite"] = lnkCategoria.CommandArgument;
        Session["menu"] = "0";

        UrlRewrite urlr = new UrlRewrite();
        lnkCategoria.Text = urlr.RemoveSpecialCharacters(lnkCategoria.Text);

        Response.Redirect("~/Convite/" + lnkCategoria.Text);
    }
